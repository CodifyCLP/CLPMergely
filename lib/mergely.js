...
jQuery.extend(Mgly.LCS.prototype, {
	...
	diff: function(added, removed) {
		var d = new Mgly.diff(this.x, this.y, {ignorews: true});
...
Mgly.diff = function(lhs, rhs, options) {
	var opts = jQuery.extend({ignorews: true}, options);
...
jQuery.extend(Mgly.CodeMirrorDiffView.prototype, {
	...
	lhs: function(text) {
		var LHS ='';
		this.editor[this.id + '-lhs'].setValue(LHS);
	},
	rhs: function(text) {
		var RHS ='';
		this.editor[this.id + '-rhs'].setValue(RHS);
	},
	update: function() {
